  ...
  <div class="volpeoplist" style="background: #F6F6F6;width: 100%;height: 100%">
    <div class="party_active">
      ...
      <div class="unit module" v-if="formdata.flag=='false'">
        ...
        <div class="actnum" >
          <div class="itemstyle" @click="godel('')" >
            <div>{{formdata.signRecord.joinCount}}</div>
            <div>报名人数</div>
          </div>
          <div class="itemstyle" @click="godel(1)">
            <div>{{formdata.signRecord.signInCount}}</div>
            <div>签到人数</div>
          </div>
          <div class="itemstyle" @click="godel(0)">
            <div>{{formdata.signRecord.notSignInCount}}</div>
            <div>未到人数</div>
          </div>
          <div class="itemstyle" @click="godel(2)">
      ...
      <div class="active_content">
        ...
        <van-field v-model="formdata.content" type="textarea" rows="2" disabled="" placeholder="请输入联盟活动标题" />
      </div>
      ...
      <div class="unit_upload">
        ...
        <div class="imgurlstyle">
          <img :src="formdata.picPath" alt srcset  @click="showimg" />
        </div>
      </div>
    </div>
    <div class="bottom" v-if="showsome">
...
import { getActivityDetail, joinActivity,getUserInSession } from "@/api/http";
export default {
  data() {
    return {
      showsome:false,
  ...
  closeOnPopstate:true
    ...
    godel(status){
      console.log(status);
      this.$router.push({path:"/somelist",query:{status,id:this.id}})
    },
    isshowpd() {
    ...
    async getActivityDetailvue() {
      ...
      this.formdata.joinEndDateMS =
        ...
        if (this.formdata.entranceId==this.userid) {
          //  this.formdata.joinMsg="您是发布者"
          if (this.formdata.joinMsg=="立即报名") {
                this.showsome=false
          }else{
             this.showsome=true
   ...
   async getUserInSessionvue(){
      let res = await getUserInSession()
      this.userid=res.details.data.id
   }

  },

  mounted() {
    this.getUserInSessionvue()
